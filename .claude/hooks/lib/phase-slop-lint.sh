...
# a numeral (a milestone-synonym plus an ordinal) in a commit subject or a
# source-code comment. The numeral is the tell; the noun alone is not.
# Idiomatic git/review/source vocabulary (Conventional Commits,
# Conventional Comments, code tags, WIP) is never flagged.
#
# Milestones in this host are content-named; there is no ordinal carve-out.
# A phase-synonym followed by a numeral is slop everywhere, including this
# host's own commit subjects.
#
# Modes:
...
# Phase-synonym nouns (the first vocab-spec section). "section" included;
...
# code (e.g. "review <code>", "finding #<n>"). The letter/`#` gate is what
...
# The host-lint binary engine OWNS this rule (its VOCABULARY + matcher), so this
# shell copy only supplements the CORE FALLBACK used when the binary is unbuilt
# (a fresh clone). The binary is the single source of truth when present - this
# avoids a duplicate that could drift from it.
...
# host-lint binary when built (exit 1 = hard tell, 0 = clean, 3 = bare-numeral
