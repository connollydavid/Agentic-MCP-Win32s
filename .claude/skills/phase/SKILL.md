---
name: phase
description: Drive a software milestone of MCP-Win32s through the full per-milestone process — planning pause, the Allium lifecycle (elicit→tend→propagate→implement→distill→weed), the merge gate + CI parity, the review gate, and close-out. Invoke to open, check the status of, advance, review, or complete a milestone. The state-aware orchestrator the host process has accreted toward.
arguments:
  - name: action
    description: "open <N> | status | gate arm|clear | review | complete <N>  (default: status)"
---

# /phase — the per-milestone orchestrator
...
|---|-------|----------------|----------------|-------------|
| 0 | Planning pause | `/phase open <N>` | milestone `README.md` reviewed, open questions settled, status → In progress, committed+pushed | this skill (will not advance from an unreviewed plan) |
| 1 | Discover | `/allium:elicit` | domain model settled, zero open questions; terminology grounded in research when naming matters | recorded in the milestone `README.md` |
...
artifacts only — the milestone-plan immediate-commit rule). These markers are
the milestone's state; `/phase status` reads them. Do not batch them.

## Implement and weed are fan-out jobs, not linear ones

The command-execution milestone was an orchestration job, not a serial slog — encode that:

**Implement — fan-out.** Decompose the milestone into independent
modules. **Freeze the interfaces first**: write and commit the `.h`
(public types + signatures) for each module before delegating, so
concurrent work cannot collide on a contract (the command-execution milestone froze `feat.h`
...
command-execution-milestone agent reported "done" with no test output and had real
failures). Integrate, then run the whole suite.

**Weed — parallel auditors.** Split the specs into clusters and
run one read-only `Explore` auditor per cluster in parallel (the command-execution milestone ran
...
2. Surface every stale reference, carried-forward correction, and open
   question; settle them by Q&A with the user. A new milestone is never just
   a preamble fixing the previous one — confirm the milestone's own scope.
3. On approval: flip the PLAN.md index and the milestone `README.md` header to
   **In progress**, commit + push (host repo; plan artifacts only).
4. Hand off to the discover stage (`/allium:elicit`).

### `status` (default)
1. Read `plan/PLAN.md` for the In-progress milestone; if none, report "no
   milestone in progress" and stop.
...
3. Address every finding **within the same PR**; record them as
   findings in the milestone `README.md`. Re-run if a fix is non-trivial.
...
4. Mark Complete in `plan/PLAN.md` and the milestone `README.md` header; append the
   milestone's lessons to `MEMORY.md` (separate commit).
5. `/phase gate clear`.
...
- **Verify platform facts, don't reconstruct them.** For any question
  ...
  this orchestrator's `/goal`, Stop-hook and PreToolUse designs all rested on
  that.
...
- Anti-slop: commit subjects and code comments are linted by the
  slop hook; keep numbered milestone-synonyms out of all code, commits,
  and docs — milestones are content-named under `plan/<NNNN-slug>/`, with
  no host carve-out.
