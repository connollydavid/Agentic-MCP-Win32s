...
Verify the device on the real OS ladder. The earlier milestones verified behaviour only under
**Wine (an NT proxy) and WSL2-native modern Windows**; every OS-tier-dependent
mechanism carries an explicit "cross-platform hardware" deferral. The device self-describes
...
From the closed milestones (quotes live in `plan/0004-command-execution/README.md` / `plan/0005-mcp-integration/README.md` / MEMORY.md):
...
- **Win32s 1.25a**: the polling/`GetExitCodeProcess` capture path (KB Q125213 —
  ...
  selection, `shared_vm` memory tier, no-threads/no-jobs/no-pty floor (the
  command-execution milestone's manual-smoke deferral).
...
2a. **Two compatibility floors to baseline against** (operator directive 2026-06-11):
   ...
   **NT 3.51** (already in-hand) is the nearest *runnable* early-NT representative if NT
   ...
   target**; whether it becomes a *run* tier in this milestone depends on sourcing + QEMU boot
...
3. **DBCS deferred**: a Japanese (cp932) Win98 will be made available later — the live
   ...
   a blocker for this milestone. The exhaustive per-codepage unit tests stand as current evidence.
...
## Environment provenance (Win32s tier base images)
...
the UTF-8 / text-encoding floor's live DBCS-safe path-scan + strict-narrow verification was
...
set locally (not git). Discs relevant to this milestone:
...
superset) is a valid run — but a strict floor check wants 1.25a. **RESOLVED 2026-06-11 — both ends now in hand**, so this milestone can verify the
...
relevant to this milestone, for DBCS-on-NT; the European ones are archival completeness).
...
practical native-Win32 target — it would exercise the NT-3.x wide-API floor that the UTF-8 / text-encoding floor
...
- **Harness + deploy bundles** (the implement-heavy item, fan-out):
...
- **Win11 native (dev host)**: device run natively so the skip-with-reason tests
  ...
  client. Folds in the exec/pty tools' VS Code demo artifacts if convenient.
- **Windows 3.11 + Win32s 1.25a (QEMU, fresh scripted install)**: build a clean,
  ...
  **unattended Windows 3.11 install** driving `SETUP /B` with a `SETUP.SHH` answer
  ...
  re-runnable script (the harness's "make the Win32s guest" target). **Stretch**:
  ...
  **🔴 The missing-MSVCRT defect — the device does not load on bare Win32s 1.25a (`MSVCRT.DLL`
  ...
  **Win32s 1.25a ships `CRTDLL.DLL`, not `msvcrt.dll`** (it's in the Win32s redist as
  ...
  `msvcrt`). **This is a CI-parity blind spot of exactly the class this milestone exists to catch:**
  ...
  a Win32s-compatible (~1995-era) `MSVCRT.DLL` redistributable (2-file deploy, and its
  ...
  **Baseline PINNED (2026-06-11).** Clean-shutdown image preserved at
  ...
  (Win32s loaded), `C:\WIN32APP\FREECELL` present. **Win32s tier status: environment ✅ complete +
  verified; device acceptance BLOCKED on the missing-MSVCRT defect (msvcrt.dll) pending a CRT-link
  ...
  **🟢 The missing-MSVCRT defect FIXED at the source (branch `claude/win32s-mini-crt`, 2026-06-11).** Root
  ...
  **🟢 The GetFileSizeEx-absence defect FIXED + DEVICE LOADS & RUNS ON Win32s (branch `claude/win32s-mini-crt`,
  2026-06-11).** With the missing-MSVCRT defect fixed, the re-deployed device cleared the MSVCRT error and
  surfaced the next one: **`GetFileSizeEx` is absent from the Win32s 1.25a thunk**
  ...
  **OPEN — serial transport on Win32s (next step; a possible further defect).** The device defaults to
  ...
  **Merge readiness.** Branch `claude/win32s-mini-crt` = the missing-MSVCRT fix (mini CRT, reviewed APPROVE)
  + the GetFileSizeEx-absence fix (`GetFileSize`) + the function-allowlist CI guard. Before merge: the GetFileSize
...
- **Win98 SE (real hardware, serial)**: matrix assert (arena/threads/manual);
...
- **Windows XP (same machine, dual-boot flip)**: matrix assert; **job objects
...
- **Close-out**: consolidated verification report committed; defects found are
  ...
  emulators out of scope); the milestone → Complete.
...
- **3 propagate**: **the per-tier checklists** — every command-execution / MCP-integration
  deferral converted to a tier × check-ID row (the carried-forward inventory above is
  the input); floor only grows.
- **4 implement**: harness fan-out (checker / bundles / serial plumbing / report template
...
- **The per-tier runs are HUMAN GATES**: each needs the user to boot/flip the
...
- **Win32s console-subsystem support**: the device is a CONSOLE-subsystem binary;
  ...
  to answer — first checkpoint of the Win32s tier; a failure becomes a remediation work item
...
✅ #37 distill — verify no-op (2026-06-12). `specs/uart.allium` was authored up front (tend), not reverse-engineered from code, so there is no spec-less module to backfill. `allium check` remains clean.
...
✅ #37 / #35 ON-TARGET ACCEPTANCE — PASS (2026-06-12, operator-driven, the pinned QEMU Win32s guest). The integrated direct-UART backend wire-responds LIVE on real Win32s — what the throwaway spike proved by hand, now as shipped device code. Deployed `mcp-w32s.exe` (sha `5636374…`, this session's strict build, mtools read-back `cmp`-verified) onto the guest `C:\`; QEMU relaunched (`run-win.bat run`, COM1 → `127.0.0.1:31800`); drove Windows 3.11 startup + `File ▸ Run C:\MCPW32S.EXE /SERIAL:COM1 /BAUD:19200` from WSL over the QEMU monitor (`mon-win.sh`, screendump-verified each step). Wire harness (`tools/phase6-qemu/wire_accept.py`) over the COM1 TCP bridge captured the **full-duplex round-trip**:
  ...
  The device launched headless under Win32s (console app, no PM window), ran the loopback-self-test + divisor-verify open ladder against QEMU's 16550A, went live, and served the round-trip cleanly (no GPF, no error dialog). **Task #35 (device wire-responding on Win32s) is satisfied by the direct-UART tier.**

✅ #37 PR #18 opened + MERGE-READINESS RE-REVIEW (2026-06-12, fresh adversarial sub-agent on the final HEAD, focused on the post-first-review delta = the two CI guards `4af7815`). **The re-review CAUGHT A REAL BLOCKER the first review could not (the guards did not exist yet then):**
  - **The fail-open-guard blocker (fixed @ d89a928): the PIN #5 no-OS-comm CI guard was FAIL-OPEN.** Its `nm` regex `' U _?(CreateFile|…)'` could not match mingw's actual dllimport thunk symbol form (`__imp__CreateFileA@28`) — the `_?` only matched the bare `_CreateFileA` form no real code emits — so it passed clean AND violating objects, defeating the very regression it advertised (a guard that "verifies the artifact" but verifies nothing is worse than none). Proven by the reviewer (injected `CreateFileA`/`SetCommState` into a uart.c copy → guard missed) and independently re-confirmed here using `serial.c.obj` as a real oracle (it legitimately references `__imp__CreateFileA@28`/`__imp__SetCommState@8`/`__imp__SetCommTimeouts@8`): the old regex MISSES them; the fixed `' U .*(…)'` MATCHES the violation AND still passes the clean `uart.c.obj` (no false-positive). Same self-applying lesson as PR #10/the escaper rule, one layer up: a claimed automated guard must be tested to FIRE on a violation, not just to pass on clean code.
  - **Everything else re-confirmed green on HEAD:** guard #1 (no-escalation) sound (fires on a crafted `cli/int/lgdt/lcall` object, zero false matches across the 17k-line app disasm); `allium check`/`analyse` clean; warning-free strict build; theft 10/10 @ 50k; `test_uart` 10/10; the 3 dispatch tests ok; the 7 `exec_*`/`ptyexec_*` failures are the known wine divergence (the diff added only the 3 `uart_*` cases); the code-level pins hold (tier gate first + sole caller + `/AUTO` re-check; FIFO exact `==0xC0`; `UartRxDrain` return-set `{>0,-1}`); scope clean; the on-target `wire_accept.py` PASS genuinely requires a `status:ok` the harness never transmits (cannot green on loopback/echo). One accepted nit (the `int $3`→`int3` fold is a trap, not an escalation vector). Verdict: **request-changes → (B1 fixed) merge-ready.** The corrected guard re-runs in CI; only the irreversible squash-merge + submodule-pointer bump await authorisation.

✅ #37 / #35 MERGED + CLOSED (2026-06-12, operator-authorised). PR #18 squash-merged to mcp-win32s `main` (`7940665`); host submodule pointer bumped `9809aab..7940665` (`5196894`). The Win32s direct-UART serial tier is banked in main — **the device wire-responds on real Win32s 1.25a**, the open question since the 0-bytes serial finding resolved (it was never a Win32s limit; the stubbed comm API was, and bare ring-3 IN/OUT on the 16550 routes around it). Tasks **#35 and #37 complete.** A post-merge note caught during close-out: the branch carried a slop commit subject (an internal review-finding code used as a name) — squashed OUT of main's permanent history (the merged subject is idiomatic), and the host anti-slop linter (`.claude/hooks/lib/phase-slop-lint.sh`) was hardened to flag the internal-code-as-name class (a review/finding/blocker noun plus a numeric or letter+digit code) so the tell is caught at commit time going forward; the same rule should port to the no-phase plugin's VOCABULARY. Remaining Win32s follow-ups are separate: **#39** (degraded-UART QEMU smoke), **#38** (the non-blocking windows-native CI job), **#33** (MSDN gap discs).

✅ #39 degraded-UART smoke — CLOSED as host-fake-covered; emulator path documented infeasible (2026-06-12, operator-decided after a feasibility spike). **Verdict: no accessible emulator can honestly exercise the detection-ladder downgrade branches, and the host fake already covers them exhaustively — so #39 adds no fidelity worth the cost.** Findings: **(1)** the QEMU we run is stock weilnetz `qemu-system-i386.exe` (11.0.50, unmodified `hw/char/serial.c`) — a **fixed working 16550A**: no `-device isa-serial` property selects the UART generation or disables the FIFO, and `FCR=0xE7`→IIR always reads `0xC0`, so **0 of 3** degraded cases (8250/16450 no-FIFO; broken non-A `IIR=0x80`; dead-clone loopback-fail) are reachable. **(2)** Patching QEMU to spoof a degraded IIR / break loopback **defeats #39's own purpose** — its stated value is real-hardware fidelity *beyond* our host fake, but a degraded model we patch in is **just another fake we authored**, epistemically no better than the C simulated-16550 already fuzzed at 50k trials. **(3)** 86Box independently models 8250/16450/16550 (`src/serial.c`), so it could cover the **no-FIFO 16450** case with genuine third-party fidelity — but the generation is tied to the emulated machine's Super-I/O (not a free selector), it still **cannot** model the *broken-non-A* or *dead-clone* (no mainstream emulator reproduces deliberately-defective silicon), and it costs a second emulator stack + guest redeploy for at best **1 of 3**. **(4)** Real vintage UART hardware is the only true fidelity for all three — out of scope. **What stands:** the host simulated-16550 fake gives **exhaustive degraded-logic coverage** (theft 50k + ASan; `nona_16550_no_fifo` pins the `0x80` case, the dead-clone pins loopback-fail-closed), and the **16550A happy path is verified live on QEMU** (the #35 acceptance). The narrow residual gap — real-*hardware* fidelity for the downgrade branches — is deferred to real silicon (out of scope), **not** silently dropped. Task #39 complete-as-documented. (Evidence: QEMU `hw/char/serial.c`; 86Box `OBattler/86Box/src/serial.c`.)

➕ #40 NT 3.1 native-Win32 floor — PLANNING PAUSE complete + OPENED (2026-06-13). Activates **decision 2a's second baseline floor** (the native-Win32/NT floor = **Windows NT 3.1 Advanced Server**, build **511**, July 1993 — "recorded as a work item, not yet a hard gate" pending sourcing, now **sourced**: `C:\Users\david\Downloads\WINNT_AS_511.ISO`, 165509120 B, vol `WINNT_AS`, multi-arch `\I386`+`\MIPS`, byte-size-identical to the in-hand `WinNT 3.10 Server` disc; `\I386\WINNT.EXE` present; **not El-Torito-bootable**, as no NT 3.x/4.0 CD is). **Acceptance (operator-settled 2026-06-13): FULL WIRE ROUND-TRIP** — the device loads+runs on NT 3.1 (gating sub-checkpoint: prints `ready`, no GPF, proving the delay-load floor reads 1993-base-Win32 correctly) **AND** wire-responds over the **OS-serial path** (`CreateFile`+`SetCommState`, which *works* on NT — unlike Win32s, where #37's direct-UART tier was needed), an echo round-trip mirroring the #35 Win32s acceptance. **Lane design (operator directive "re-use in a different lane our Win32s assets"): a separate, isolated NT lane that borrows the tooling but never touches the Win32s guest** — own disk `vendor/winnt31/build/hdd.img` (the Win32s `vendor/win311/build/hdd.img` is untouched), own ports (serial **31801** / monitor **55556** / VNC **:1**) so both guests can coexist, a thin launcher (the existing `run-win.bat`/`run.sh` already take `SERIAL_PORT`/`MON_PORT`/`VNC_DISP` + a build-dir override); **reuses as-is/by-pattern** the `dos622-boot.img` boot floppy, the mtools FAT-image build (`make-installdisk.sh` → an `I386` data disk, the `install-d.img` pattern), and the `mon-win.sh` monitor driver + `wire_accept.py` harness. **Install approach:** boot DOS 6.22 → run `\I386\WINNT.EXE /B` (floppyless — copies boot files to HDD, reboots into NT text-mode setup, no fabricated NT setup floppies), with `I386` copied onto a **FAT HDD image** so `WINNT /B /S:D:\I386` runs with **no MSCDEX/CD driver**. **NT-3.1 QEMU tuning** (it is the hardest NT to emulate): `-cpu 486`, `-vga cirrus` (NT 3.1 ships a Cirrus driver → usable GUI), a **small FAT system partition** (boot files within the first 1024 cylinders), modest RAM. **Risk + fallback (decision 2a + operator 2026-06-13 "consult before pivoting"):** if NT 3.1 proves too hardware-picky under QEMU after reasonable effort, **STOP and consult** before any pivot to **NT 3.51** (the nearest *runnable* early-NT, carried in the MSDN Jan-1998 set) or a secondary emulator (86Box/PCem). **Human-gated** like the Win32s runs: QEMU runs on the **Windows host** (the WSL sandbox reaps `qemu*`); the agent prepares the lane and drives the install via the monitor. Tracked as **task #40**. Execution begins after this record is pushed.

✅ #40 NT lane staged + media verified — 2026-06-13. **ISO `WINNT_AS_511.ISO` sha256 `940dcefd46e1fbeebf27b2bc1befcbd75867e4cd41cf639e36c116b371684d1b` (165509120 B)** verified **genuine + nothing unofficial** (decision 6): contents are period-correct MS **Windows NT 3.1 build 511** — every file dated **1993-07-24**, the standard dual-arch `\I386`(x86)+`\MIPS` layout, a valid setup tree (`WINNT.EXE`/`TXTSETUP.INF`/`SETUPLDR`/`NTDETECT.COM`/`DOSNET.INF`), a plain install CD with **no third-party preloads** (contrast the rejected contaminated OVA). Integrity anchored on sha256; authenticity period-correct-but-not-independently-referenced → **integrity ✓ / auth-TBC**, the same posture as the Win32s media. **Install mechanism verified from the disc itself** (not assumed): the CD is **not El-Torito-bootable**; `WINNT.EXE` has **no `/B`** (its usage is `WINNT [/S:src][/T:tmp][/I:inf][/X|[/F][/C]]` — floppyless came in NT 3.5); **no floppy image ships** on the CD (the `DISK1/DISK2` entries are 2-byte volume tags) — so install is DOS → `WINNT /S:D:\I386`, which copies to a C: temp dir and **writes a Setup boot floppy**, then reboot from it into NT text setup. **Lane assets built deterministically** (WSL, qemu-img/mtools/7z — no VM): `tools/phase6-qemu/build-nt31.sh` → `vendor/winnt31/build/` = `hdd.img` (C:, 500 MB bootable-FAT, unformatted — operator `FORMAT C: /S`), `install-i386.img` (D:, 64 MB FAT16 = the `\I386` tree, 1211 files / 34 MB, `WINNT.EXE` present), `floppies/dos622-boot.img` (**reused** from the win311 lane), `floppies/ntsetup-boot.img` (blank 1.44 MB for WINNT to write). Launcher `tools/phase6-qemu/run-nt-win.bat` (`-cpu 486 -vga cirrus -net none`; isolated ports COM1→**31801**, monitor→**55556**, VNC **:1** — coexists with the win311 lane); operator run-book appended to `tools/phase6-qemu/README.md`; reuses `mon-win.sh` + `wire_accept.py`. **Next: operator-driven boot + install on the Windows host** (QEMU can't run in the WSL reaper), then the #35-equivalent wire round-trip over the NT OS-serial path.

✅ #40 NT 3.1 INSTALLED + device LOADS & RUNS on the floor; three floor findings fixed (2026-06-13, operator-driven QEMU lane, parked mid-acceptance). **Windows NT 3.1 Advanced Server installed, booted, and logged in** under QEMU (the separate `winnt31` lane), driven from WSL over the monitor. Install lessons worth keeping: NT 3.1 is the hardest NT to emulate but installs with `-cpu 486 -vga cirrus`; the CD is not bootable + `WINNT.EXE` has no `/B`, so it's DOS → `WINNT /S:D:\I386` → it writes a Setup boot floppy → reboot into text setup; **`sfdisk` writes no MBR bootstrap so C: needs `FDISK /MBR`** before `FORMAT C: /S` (else SeaBIOS hangs at "Booting from Hard Disk"); NT 3.1 **Advanced Server mandates a domain role** (no workgroup) and needs a working NIC, so the lane attaches an **NE2000 ISA** (`ne2k_isa`, IO 0x300/IRQ 3 = NT's "Novell NE2000 Compatible" defaults) and the install picks "Controller in New Domain" (domain `MCP`). Launcher shows a **desktop window** (not VNC) so the operator can watch while the agent drives via the monitor.
  **THREE real native-Win32-floor findings** — each a load-time gate the Win32s/Win9x tiers structurally never surfaced (their loaders don't enforce the PE stamp; Win32s 1.25a (1995) is *newer* than NT 3.1 (1993) and backported the missing APIs). Fixed on submodule branch **`claude/...` `nt31-pe-version-stamp` @ `766e3c8`** (pushed; NOT merged — the Allium lifecycle + review gate are pending before it can land):
  - **F1 — PE version stamp 4.0.** mingw stamped OS/subsystem version 4.0; NT 3.1's loader refuses a newer stamp with **`ERROR_BAD_FORMAT` ("Unexpected DOS error: 11")**. Fix: stamp **3.10** in the link step (`-Wl,--major-os-version,3 …--major-subsystem-version,3 --minor=10`; MSVC `/SUBSYSTEM:CONSOLE,3.10`) — 3.10 still loads on every later Windows.
  - **F2 — `SetHandleInformation` static import** (arrived NT 3.51/Win95) — **"entry point not found in KERNEL32.dll"**. Used in `exec_ops.c` to clear `HANDLE_FLAG_INHERIT` on parent-only pipe ends (Q5). Fix: route through the **`feat.c` capability probe** (`pSetHandleInformation`) with a `ClearHandleInherit` helper that falls back to the classic **`DuplicateHandle` non-inheritable-copy** idiom on NT 3.1.
  - **F3 — `lstrcpynA` absent from NT 3.1's kernel32** (only `lstrcpy`/`A`/`W` exist; proven by diffing the device's 55 kernel32 imports against the disc's 554 kernel32 exports). It was the codebase's bounded-copy primitive (~50 sites/8 files). Fix: supply **`McpStrCpyN`** (`src/strutil.c`, DBCS-aware via `CharNextA`) and drop the import.
  **Verified:** the fixed `mcp-w32s.exe` still imports **only kernel32 + user32**, stays i386/console, version-stamped 3.10; full build green; every `McpStrCpyN`-affected suite green; `test_serial`/`test_exec_ops` fail **identically with and without the change** (stash-baseline proven — the known wine divergence, WSL interop down, not a regression). **Live on NT 3.1 AS the device now loads and RUNS** (console window opens, past loader + imports) — the **device-loads-and-runs gating sub-checkpoint is MET**.
  **OPEN (parked):** the full wire round-trip is blocked on a **runtime** issue — the device takes the NT OS-serial path correctly (not direct-UART) but **`CreateFileA` on COM1 fails ("failed to open serial port")**. Root cause TBC: NT 3.1 COM1 detection/enumeration, or the port-name form `OpenSerialPort` passes to `CreateFileA` (`serial.c:70-78`), or an IRQ/config interaction. Next session: resolve the COM1 open, then the `wire_accept.py` round-trip (port 31801, now tier-neutral). The three fixes then go through tend/propagate/weed/review before merge. Task **#40 stays in progress.**
...
- Full OpenAI agent-loop demo (unchanged from the exec/pty tools — optional, user-run, needs a key).
- MSDN Jan-1998 gap discs 6/7/8/15 (task #33 — document-and-defer + closed 2026-06-12).
  True part numbers + contents could **not** be verified from any primary source: not
  on archive.org, and the extrapolated part #s (X03-54212/13/14/21) were RETRACTED as
  unverified (X03-54212 is a 1996 product; the set likely carried over unchanged discs
  from an earlier quarterly pressing under an older part #/date). No agent-actionable
  path — verification needs the set's own disc-1 catalog, a redump.org set manifest, or
  physical disc labels (operator-led sourcing). Low priority for this milestone: the core Win32s media
  is already in hand and hash-pinned. Re-open only if discs or a primary manifest surface.
