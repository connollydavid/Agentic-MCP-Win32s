...
✅ Phase 6 / #40 NT 3.1 ON-TARGET ACCEPTANCE — **PASS** (2026-06-15, operator-driven QEMU lane; agent-driven via the monitor). The device **loads, runs, and wire-responds on Windows NT 3.1 build 511** over the **OS-serial path** (`CreateFile`+`SetCommState`, not direct-UART — `is_nt`, correctly). `wire_accept.py` on COM1 (TCP 31801) captured the full duplex round-trip: ready line `{"status":"ready",...,"version":"Windows 3.10.511 (NT)","transport":"serial","features":{"is_nt":true,"mem":"process","encoding":"utf8_via_w","ctrl_events":true,"threads":true,...}}` → cmd `{"cmd":"echo","id":"acc1","line":"HELLO-WIN32S-UART"}` → resp `{"id":"acc1","status":"ok","data":"HELLO-WIN32S-UART"}`. Console clean (no GPF/dialog). Evidence: `vendor/winnt31/build/accept/`.
  **The COM1-open blocker, root-caused by verification not guessing.** The parked "CreateFileA fails" was resolved with the instrumented build: it reported **error 87 (`ERROR_INVALID_PARAMETER`)** — ruling out the not-enumerated (error 2) and name-form hypotheses. NT's own **`MODE`** (independent OS oracle) opened COM1 and accepted `19200/n/8/1`, proving the port + OS are fine and the fault was in the device's open sequence. That pinpointed **`SetCommState`, not `CreateFile`**: the device built its `DCB` from a `memset(0)` and called `SetCommState` cold, leaving `XonChar == XoffChar == 0` — which NT 3.1's 1993 serial driver rejects (modern NT is lenient). The `\\.\COMn` name fallback was the wrong lever (both forms resolve the same device); the **error surfacing** was the right one, and its first label ("CreateFileA error") was itself a confident-wrong claim, corrected. **Fix (branch `nt31-pe-version-stamp` @ `05d265c`):** `GetCommState`-first then modify only baud/bytesize/parity/stop + flow flags (the idiom MODE/Terminal use), plus per-stage open-error reporting (CreateFileA/GetCommState/SetCommState/SetCommTimeouts). No new import (`GetCommState` is kernel32); object opcode-clean; `\\.\COMn` fallback retained.
  **#40 now carries four native-Win32-floor fixes, all proven on-target:** F1 PE-stamp 3.10, F2 `SetHandleInformation`→fallback, F3 `lstrcpynA`→`McpStrCpyN`, and the COM1 `GetCommState`-first open. The operator-gated acceptance is **done**; remaining before merge: the **Allium lifecycle** (tend/propagate/weed) + **review gate** + observed CI green + merge authorization. Task **#40 stays in progress** (lifecycle pending).

✅ 6.2 #40 lifecycle — tend → propagate → implement → weed (2026-06-16, branch `nt31-pe-version-stamp`). **tend:** the four fixes are correctness fixes restoring already-spec'd behaviour; `serial.allium` `PortOpens`/`PortOpenFails` already models the open (the `GetCommState`-first change is below the abstraction); zero spec changes; `allium check` clean. **propagate:** surfaced two real test gaps — `McpStrCpyN` (F3, used ~50 sites) had **no** direct test, and F2's `DuplicateHandle` fallback (NT-3.1-only) was unexercised (the echo acceptance spawns no child). **implement (gaps filled, sub-agent + orchestrator-verified):** a host theft PBT (`theft_strutil.c`, 5 properties × 50k, ASan — terminated/prefix/no-split/bounded/edges, with a cp932 `CharNextA` shim so the DBCS-no-split property is deterministic) + a `prop.h` on-target mirror; `ClearHandleInherit` fallback + fail-closed tests. Also fixed a pre-existing host-pbt breakage (the `theft_catalog` build line lacked `src/strutil.c` → undefined `McpStrCpyN` since the F3 swap). **weed (adversarial, gate-bypass dimension):** ZERO spec↔code drift; tier gate (SECURITY PIN #1) holds — `GetCommState`-first sits below `UartTierWantsDirect()`, direct-UART unreachable off-Win32s by construction. The gate-bypass pass found `ClearHandleInherit` **failed open** (both routes ignored failure → a child could inherit a parent-only pipe end); **hardened fail-closed** (operator-decided): returns BOOL, the spawn aborts before `CreateProcessA` if any clear fails, pinned by a stubbed-failure test.

✅ 6.2 #40 CI parity — green on `b4719be` (2026-06-16, observed: `build-and-test` + `bridge` + `conformance` + `real-Windows`). **Caught a regression hidden since `766e3c8` (2026-06-13):** CI had been **red** the whole time, but only at the **function-level Win32s import allowlist** step (host-pbt, build, ctest-under-Wine, and the other jobs all passed) — confirming the local `exec_*`/`ptyexec_*` failures are the WSL-interop-down Wine divergence, NOT a regression. The import set had drifted: F2 made `SetHandleInformation` a `GetProcAddress` probe and added `DuplicateHandle`; F3 dropped `lstrcpynA`; the COM1 fix added `GetCommState`. Reconciled `tools/win32s-import-allowlist.txt` (+`DuplicateHandle` +`GetCommState`, −`SetHandleInformation` −`lstrcpynA`) — both additions verified present in the Win32s 1.25a thunk (`W32SCOMB.DLL` exports `GetCommState`[697] + `DuplicateHandle`[2015], per the file's baseline-image method). Lesson: a long-lived feature branch can sit CI-red on a single guard while every other signal is green — **observe the actual CI run, do not infer green from local + the load test.**

✅ 6.2 #40 review gate — **APPROVE**, zero findings (2026-06-16, fresh adversarial read-only sub-agent over the full 6-commit branch). Independently re-ran and OBSERVED: warning-free strict C89/i386 build; `ctest` 20/20 (the reviewer's host had WSL interop up — the `exec_*`/`ptyexec_*` cases passed, confirming the local red was the interop-down Wine divergence, not a regression); `allium check` clean; host-pbt ALL PASS (`theft_strutil` 5×50k under ASan); imports `{kernel32,user32}` with an exact allowlist match; no forbidden opcodes in the changed objects; PE stamp 3.10 in the binary. Both security boundaries refuted-and-held: the tier gate (`GetCommState`-first below `UartTierWantsDirect()`; `/AUTO` re-checks) and the fail-closed handle isolation (aborts before `CreateProcessA`, pinned by a test that fails on the prior fail-open code). The 50-site `lstrcpynA`→`McpStrCpyN` swap verified mechanical (no off-by-one; `n` semantics identical). 0 blocker / 0 should-fix / 0 nit; 3 benign observations. **Merge gate now fully satisfied** (lifecycle + CI green + review approve); only the operator-authorised squash-merge + pin bump remain.
...
- MSDN Jan-1998 gap discs 6/7/8/15 (task #33 — document-and-defer + closed 2026-06-12).
  ...
  physical disc labels (operator-led sourcing). Low Phase-6 priority: the core 6.2 media
