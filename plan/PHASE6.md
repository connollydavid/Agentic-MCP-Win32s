...
⏳ 6.2 #37 implement — interface FROZEN (branch `claude/win32s-uart` @ bea05ce, pushed). `src/uart.h` is the frozen contract for the fan-out: the injected `UartPortIo` port-I/O seam, the `UartDriver` state, the pure detection-ladder/driving entry points (`UartDetect`/`UartProgramDivisor`/`UartLoopbackSelfTest`/`UartOpenSequence`/`UartTxChunk`/`UartRxDrain`/`UartDrainAndClose`), and behind `#ifndef UART_HOST_PURE` the tier gate `UartTierWantsDirect()` + `UartBackendOpenDirect()`. Doc comments pin each function to its `uart.allium` rule/invariant + the six security pins. Compiles clean `-std=c89 -pedantic -Werror` host-pure; no `.c` yet (build unchanged). **NEXT — the fan-out** (a natural fresh-session boundary for this context-heavy, security-critical stage): sub-agent A = `src/uart.c` pure section + `tests/host/theft_uart.c` + the simulated-16550 fake (transcribed from the spike); sub-agent B = `tests/test_uart.c` (`prop.h` mirrors) + `tests/test_serial.c` dispatch-gate; **main session** = the `uart.c` `#ifndef UART_HOST_PURE` real backend (x86 `inb`/`outb` asm + `UartBackendOpenDirect` + `UartTierWantsDirect`), the one-line `serial.c` dispatch (`if (UartTierWantsDirect()) return UartBackendOpenDirect(...)` first in `SerialBackendOpen`), the `CMakeLists.txt` wiring (`uart.c` in CORE_SOURCES + a `-DUART_HOST_PURE` host target + `mini_crt` `-ffreestanding -fno-builtin` + `uart.o` opcode grep), and the import-guard `awk` ordinal fail-closed hardening; reuse `stash@{0}` /BAUD. Then build + independently re-test, gate-arm at first all-green → distill → weed → review → merge → on-target #35 acceptance.
