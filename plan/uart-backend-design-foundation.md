...
Foundation for the Win32s direct-UART transport backend (the direct-UART / cross-platform work, **task #37**),
...
1. **Presence (IER store-test)** — save IER; write `0x00`→+1, read `&0x0F` expect `0x00`;
   ...
   bus reads `0xFF`). (Linux `autoconfig` first probe.)
2. **FIFO probe** — write `FCR = 0xE7` (enable+clear+64-byte bit), read `IIR & 0xC0`:
   ...
   (FIFO **broken → treat as no-FIFO**); `0x00` → no FIFO → scratch test.
