...
- **5.0** — bridge core: rmcp, stdio, tools-only, lifecycle/negotiation; mock-device harness. **tend ✅** (`specs/mcp-bridge.allium`, mcp-win32s `531e9de`, allium check/analyse clean) · **propagate ✅** (`bridge/OBLIGATIONS-5.0.md`, 54 obligations → proptest/integration/Inspector, `a5b154a`) · **implement ✅ first-green** (`bridge/` Rust crate on rmcp 1.7; `cargo test` 8 green = 5 integration incl. full MCP lifecycle over duplex + 3 proptest; **MCP Inspector CLI conformance green** — `tools/list` shows `win32_echo` with a 2020-12 inputSchema, `tools/call` round-trips to `{content, structuredContent, isError:false}`). Pending: extend `/phase-gate` to run the bridge `cargo test` (a Stop-hook edit needing explicit authorization), then arm; then weed/review (the merge gate).
